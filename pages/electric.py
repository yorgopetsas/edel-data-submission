...
previous_button = st.button(label=edit_general_info_l, use_container_width=True)
if previous_button:
...
with st.container(border=True):
	...
	if motor_type == motor_types[1]:
		...
		with col3:
			...
			potencia_motor_cv = st.text_input(label=potencia_motor_cv_l)
			potencia_motor_kw = st.text_input(label=potencia_motor_kw_l)
			consumo = st.text_input(label=consumo_l)
			break_tension = st.text_input(label=break_tension_l)
		with col4:
			rpm = st.text_input(label=rpm_l)
			vel = st.text_input(label=vel_l)
			enconder_type = st.text_input(label=enconder_type_l)
	...
	with man1:
		wardrobe_el = st.selectbox(label=wardrobe_l, options=wardrobes_el)
		mabra = st.selectbox(label=mabra_l, options=maniobras)
	with man2:
		mot_second = st.text_input(maniobra_motor_l, value=motor_type)
		restate = st.selectbox(label=restate_l, options=wardrobes_el)
...
if door_finished:
	with st.container(border=True):
		...
		with datos3:
			a3_standart = st.selectbox(label=a3_standart_electric_l, options=a3_standarts_electric)
...
if submit_button:
	...
	if motor_type == "Gearless":
		labels_list = initialize_labels_list_gearless()
	elif motor_type == "Geared":
		labels_list = initialize_labels_list_geared()
